...
async function run() {
  ...
  if (!args[2]) {
    console.log(chalk.red('No seed accept as arg!'));
    return;
  }
  ...
  const scmlTemplate = FS.readFileSync(
    PATH.join(path, 'veggie.scml'),
    'utf8'
  ).toString();
  const scmlText = scmlTemplate.replace(
    new RegExp(`\\[VEGGIE\\]`, 'g'),
    veggieName
  );
  FS.writeFileSync(
    PATH.join(exportFolderPath, `${veggieName}.scml`),
    scmlText,
    'utf8'
  );
  ...
  FSE.copySync(
    originPath,
    PATH.join(exportResFolderPath, `${veggieName01}-0.png`)
  );
  FSE.copySync(
    cookedPath,
    PATH.join(exportResFolderPath, `${veggieName01}-1.png`)
  );
  ...
  originImg
    .resize(64, 64)
    .write(
      PATH.join(path, '..', 'images', 'inventoryimages', `${veggieName}.png`)
    );
