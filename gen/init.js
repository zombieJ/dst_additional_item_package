...
function run() {
  ...
  const DST_TOOL_ORI = "Don't Starve Mod Tools";
  const DST_TOOL_SYMBOL = "DSTMT";

  const steamPath = path.resolve(prjPath, "..", "..", "..");

  const targetName = fs.existsSync(path.resolve(steamPath, DST_TOOL_SYMBOL))
    ? DST_TOOL_SYMBOL
    : DST_TOOL_ORI;
