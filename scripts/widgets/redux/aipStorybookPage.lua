...
function CookbookPageCrockPot:CreateDesc(index)
	...
	self.descHolder = self.root:AddChild(Widget("descHolder"))
	...
	for i, descInfo in ipairs(descList) do
		...
		if type(descInfo) == "string" then -- 文本内容
			local text = self.descHolder:AddChild(Text(UIFONT, 30))
		...
		elseif descInfo.type == "img" then -- 图片
			...
			local img = self.descHolder:AddChild(Image(atlas, image))
